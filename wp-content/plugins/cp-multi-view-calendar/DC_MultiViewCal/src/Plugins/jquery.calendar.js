...
; (function($) {
    ...
    $.fn.bcalendar = function(option) {
        var def = {
             ...
             */
            weekstartday: 1,  //start from Sunday by default
            showtooltip:false,
            tooltipon:1,
            shownavigate:false,
            navigateurl:"",
        ...
        function adaptWH()
	    {
	        $(".multicalendar").each(function(){
                    ...
                    $('.calendar-border').width($('#multicalendar').width())
                });
	    }
        ...
        //contruct DOM
        function render() {
            console.log($('#footer-calendar-text').html($('#txtdatetimeshowcal1_0').html()))
            if (f == 0) {
            console.log($('#multicalendar').width(), $('#multicalendar').height())
            $('#multicalendar').append('<div class="calendar-border cs-6" style="height:' + ($('#multicalendar').height()+15) + 'px; width:' + ($('#multicalendar').width() + 20) +'px; "> </div>')
                f = 1
        }
            //params needed
            ...
            var config = { view: option.view, weekstartday: 1, theme: option.theme,thecontainer: option.thecontainer };
            if (option.view == "day" || option.view == "week" || option.view == "nDays" || option.view == "rowMonth") {
        ...
        function BuildMonthView(showday, events, config) {
            ...
            html.push(cc);
            //build header
            html.push("<div id=\"mvcontainer"+config.thecontainer+"\" class=\"mv-container\"><div id='daynames-container'>");
        ...
        function BuilderMonthBody(htb, showday, startday, events, bodyHeight,config) {
            ...
            for (var j = 0; j < rc; j++) {
                ...
                //title tr
                htb.push("<tr>");
                var titletemp = "<td display=\"${display}\" class=\"st-dtitle${titleClass}\" ch='qkadd' abbr='${abbr}' axis='00:00' title=\"${title}\"><span class='monthdayshow'>${dayshow}</span><i class='fas fa-star monthdayshow-star'></i></a></td>";

                var newevents = []
                for (var i = 0; i < formatevents.length; i++){
                    newevents = newevents.concat(formatevents[i])
                }
                neweventslist = []
                for (i = 0; i < newevents.length; i++){
                    if (newevents[i] != undefined){
                        neweventslist.push(newevents[i])
                    }
                }
                ...
                for (i = 0; i < 7; i++) {
                    ...
                    var f = false
                    if (neweventslist){
                        for (var jj = 0; jj < neweventslist.length; jj++){
                            if (new Date(dateFormat.call(day, "yyyy/MM/dd")).toDateString() == new Date(neweventslist[jj][0]['daystr']).toDateString()){
                                f = true
                            }
                        }
                    }
                    if (f) {
                        o.titleClass = " st-dtitle-event";
                    }
        ...
        function dayshow(e, data) {
            ...
            $('#pop-up__calendar').css('display', 'block')
            $('.btn-close').click(function(){
                $('#pop-up').css('display', 'none')
                $('#pop-up__calendar').css('display', 'none')
            })
            $('.pop-up__shadow').click(function(){
                $('#pop-up').css('display', 'none')
                $('#pop-up__calendar').css('display', 'none')
            ...
            for (let i = 0; i < option.eventItems.length; i++){
				if (option.eventItems[i][0] == id){
					$('#pop-up__calendar').html('<div class="pop-up__header"><h2>В этот день</h2><button class="btn-close" onclick="$(\'#pop-up\').css(\'display\', \'none\')" type="button"></button></div>')
					$('#pop-up__calendar').append('<h2>' + option.eventItems[i][1] + '</h2>')
					if(option.eventItems[i][11]){
						$('#pop-up__calendar').append('<p>' + option.eventItems[i][11] + '</p>')
					}
					$('#pop-up__calendar').append('<div class="venzel"></div>')
				}
            ...
            if (data != null) {
                if (option.quickDeleteUrl != "" && data[8] == 1 && ( (option.readonly != true) || (option.readonly == true && option.showtooltipdwm) || (option.showtooltipdwm_mouseover)   )) {
                    /* 
                    var csbuddle = '<div id="bbit-cs-buddle" class="cs-6">'
